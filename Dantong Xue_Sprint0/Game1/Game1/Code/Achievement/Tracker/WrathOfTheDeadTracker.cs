...
{
    class WrathOfTheDeadTracker : ITracker
    {
        private Game1 game;
        ...
        private int goalKills = 10;
        private int healthThreshold = 1;
        private int prevKills = 0;

        public WrathOfTheDeadTracker(Game1 game)
        {
            this.game = game;
            texture = AchievementFactory.GetWrathOfTheDead();
        ...
        public bool Update(bool startDrawing, int x, int y)
        {
            ...
            if (!completed)
            {
                if (game.link.itemList["Heart"] <= healthThreshold) 
                {
                    currentKills = Enemy.DrawAndUpdateEnemy.numberOfKilled() - prevKills;
