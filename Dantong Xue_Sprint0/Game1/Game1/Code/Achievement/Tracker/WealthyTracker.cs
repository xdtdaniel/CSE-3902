...
namespace Game1.Code.Achievement.Tracker
{
    class WealthyTracker : ITracker
    {
        ...
        private int goalNum = 99;

        public WealthyTracker(Game1 game)
        {
            this.game = game;
            texture = AchievementFactory.GetWealthy();
        ...
        public bool Update(bool startDrawing, int x, int y)
        {
            ...
            if (!completed)
            {
                if (game.link.itemList["Ruby"] >= goalNum) 
                { 
                    completed = true;
                }
