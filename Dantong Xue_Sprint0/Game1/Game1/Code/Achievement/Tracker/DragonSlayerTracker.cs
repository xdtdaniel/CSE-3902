...
{
    class DragonSlayerTracker : ITracker
    {
        private Game1 game;
        ...
        private string goalEnemyName = "aquamentus";

        public DragonSlayerTracker(Game1 game)
        {
            this.game = game;
            texture = AchievementFactory.GetDragonSlayer();
        ...
        public bool Update(bool startDrawing, int x, int y)
        {
            ...
            if (!completed)
            {
                if (Enemy.DrawAndUpdateEnemy.GetKilledEnemyName() == goalEnemyName) { 
                    completed = true;
                }
