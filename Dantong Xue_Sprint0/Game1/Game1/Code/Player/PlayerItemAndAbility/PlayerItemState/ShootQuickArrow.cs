...
namespace Game1.Code.Player.PlayerItem.PlayerItemState
{
    class ShootQuickArrow : IPlayerItemState
    {
        ...
        private int hit_x;
        private int hit_y;
        private int[] hitWidth = { 5 * scale, 10 * scale, 15 * scale };
        private int[] hitHeight = { 5 * scale, 10 * scale, 15 * scale };
        ...
        public ShootQuickArrow(Link link, float angle, int x, int y)
        {
            ...
            arrow_x = x + link.linkWidth / 2 + rd.Next(minRange, maxRange);
            arrow_y = y + link.linkHeight / 2 + rd.Next(minRange, maxRange);
            ...
            ring = PlayerAbilityFactory.Instance.GetBurstRing()[2];
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            {
                sourceRectangle = new Rectangle(0, 0, ring.Width, ring.Height);
                destinationRectangle = new Rectangle(ring_x, ring_y, ringWidth, ringHeight);
                origin = new Vector2(ring.Width / 2, ring.Height / 2);


                spriteBatch.Draw(ring, destinationRectangle, sourceRectangle, Color.White, angle, origin, SpriteEffects.None, 1);
            }
